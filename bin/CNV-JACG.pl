...
my ( $Help, $precnv, $bams, $ref, $outdir );
...
die `pod2text $0` if ( $Help or !$ref or !$precnv or !$outdir or !$bams );
$bams =~ s/\s+//g;
my @allbam = (split /,/, $bams);
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
my $snps      = "$home/lib/chr21.maf.sort.txt.gz";
...
our $bam = "";
for $bam ( @allbam ) {
    $sample = basename ($bam); $sample =~ s/\.bam$//g;
    $precnvbase = join "-", ($sample, basename($precnv));
    # Calculate het probability
    &showLog("Calculating het probability for $sample...");
    ...
    &showLog("Calculating mean depth in fixed region for $sample...");
    ...
    &showLog("Calculating repeat percentage for each CNV for $sample...");
    `$bedtools intersect -wao -a $precnv -b $repeat/chr21_SD_N_region_RepeatMasker_SimpleRepeat_merge.bed $repeat/N_region > $outdir/$precnvbase.repeat`;
    ...
    &showLog("Calculateing depth and GC for each CNV for $sample...");
    ...
    &showLog("Getting other features for $sample...");
...
=head1 Options
    ...
    -b|-bam      [s]    bam file(s) (if muliple bam files, separated by comma <-b bam1,bam2>)
